...
public class PdfGraphics2DExt extends Graphics2D {
    ...
    private static final class Kid {
        final int pos;
        final PdfGraphics2DExt graphics;
        Kid(int pos, PdfGraphics2DExt graphics) {
            this.pos = pos;
	...
	private PdfGraphics2DExt(PdfWriter pdfWriter) {
		this.pdfWriter = pdfWriter;
	}
	
    public PdfGraphics2DExt(PdfWriter pdfWriter, PdfContentByte cb, final float width, final float height) {
    	this(pdfWriter, cb, width, height, null, false, false, 0);
    }

    public PdfGraphics2DExt(PdfWriter pdfWriter, PdfContentByte cb, final float width, final float height, final FontMapper fontMapper) {
    	this(pdfWriter, cb, width, height, fontMapper, false, false, 0);
    }
    
    public PdfGraphics2DExt(PdfWriter pdfWriter, PdfContentByte cb, final float width, final float height, final boolean onlyShapes) {
    	this(pdfWriter, cb, width, height, null, onlyShapes, false, 0);
    }
    
    /**
     * Constructor for PDFGraphics2D.
     */
    public PdfGraphics2DExt(PdfWriter writer, PdfContentByte cb, float width, float height, FontMapper fontMapper, boolean onlyShapes, boolean convertImagesToJPEG, float quality) {
        super();
...
*/
    ...
    public Graphics create() {
        PdfGraphics2DExt g2 = new PdfGraphics2DExt(pdfWriter);
    ...
    private void internalDispose(ByteBuffer buf) {
        ...
        if (kids != null) {
            for (Kid kid: kids) {
                pos = kid.pos;
                PdfGraphics2DExt g2 = kid.graphics;
                g2.cb.restoreState();
                g2.cb.restoreState();
...
/*	private com.itextpdf.text.Image createPNG(Image img) throws IOException, BadElementException  {
    ...
    public boolean drawPDfImage(com.itextpdf.text.Image image, AffineTransform xform) {
        ...
        xform.scale(image.getWidth(), image.getHeight());
